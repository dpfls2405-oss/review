...
def load_data():
    try:
        ...
        for ym in dates:
            for b in brands:
                for s in ["IBLE","VIM","AROUND","GX","T20","RINGO"]:
                    rows.append({
                        'ym': ym,
                        'brand': b,
                        'series': s,
                        'combo': f"{s[:3]}-{b[:2]}",
                        'name': f"{s}품목",
                        'forecast': np.random.randint(100, 800),
        ...
        for ym in dates:
            for b in brands:
                for s in ["IBLE","VIM","AROUND","GX","T20","RINGO"]:
                    a_rows.append({'ym': ym, 'combo': f"{s[:3]}-{b[:2]}", 'actual': max(0, int(np.random.normal(400, 150)))})
...
# -----------------------
st.sidebar.title("메뉴")
page = st.sidebar.radio("페이지 선택", ("시계열 추이", "시리즈 상세", "전체 데이터"))

st.sidebar.markdown("---")
st.sidebar.title("공통 필터")
...
def page_time_series():
    ...
    mg_sel_month = mg_all[(mg_all['ym'] == sel_ym) & (mg_all['brand'].isin(sel_br))]
