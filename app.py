        ...
        for ym in dates:
            for b in brands:
                for s in ["ACCESSORY","IBLE","SPOON","SODA","T60","RINGO","T20","GX","AROUND","PLT"]:
                    # ★ 핵심 수정: 모든 공급단 값을 골고루 배분 (NaN 포함)
                    supply_val = np.random.choice(
                        ['시디즈제품', '의자양지상품', '베트남제품', np.nan],
                        p=[0.3, 0.3, 0.3, 0.1]
                    )
                    rows.append({
                        'ym': ym,
            ...
            for b in brands:
                for s in ["ACCESSORY","IBLE","SPOON","SODA","T60","RINGO","T20","GX","AROUND","PLT"]:
                    a_rows.append({
                        'ym': ym,
                        'combo': f"{s[:6]}-{b[:2]}",
                        'actual': max(0, int(np.random.normal(800, 600)))
                    })
...
# -----------------------
# ★ 핵심 수정 1: 메인 대시보드 필터를 expander로 감싸기
# -----------------------
with st.sidebar.expander("메인 대시보드 필터", expanded=True):
    sel_supply_main = st.selectbox("공급단 선택 (메인)", supply_options, index=0,
                                   key="supply_main")

    # 시리즈 목록 (브랜드 선택에 따라 동적)
    available_series = sorted(
        f_df[f_df['brand'].isin(sel_br)]['series'].dropna().unique().tolist()
    )
    ...
    if series_count == 0:
        ...
        sel_sr_main = st.multiselect("시리즈 선택 (메인)", available_series,
                                     default=available_series, key="series_main")
    else:
        search_series_main = st.text_input("시리즈 검색 (메인)", key="search_series_main")
        top_n_default_main = 20
        series_agg_all = (
            f_df[f_df['brand'].isin(sel_br)]
            .groupby('series')
            .agg({'forecast': 'sum'})
            .reset_index()
        )
        top_series_main = (
            series_agg_all.sort_values('forecast', ascending=False)
            .head(top_n_default_main)['series'].tolist()
        )
        if search_series_main:
            filtered_series_main = [s for s in available_series
                                    if search_series_main.lower() in s.lower()]
            sel_sr_main = st.multiselect("검색 결과에서 선택 (메인)",
                                         filtered_series_main,
                                         default=filtered_series_main[:top_n_default_main],
                                         key="series_main_search")
        else:
            sel_sr_main = st.multiselect("시리즈 선택 (메인 기본 상위)",
                                         available_series,
                                         default=top_series_main,
                                         key="series_main_top")

    sort_metric_main = st.selectbox(
        "정렬 지표 (메인)",
        ["차이량(|실-예측|) 큰 순", "차이량(실-예측) 큰 순",
         "실수주량 큰 순", "예측수요 큰 순", "달성률 큰 순"],
        key="sort_main"
    )
    top_n_main = st.slider("Top N 표시 수 (메인)", 5, 50, 10, key="topn_main")
    search_term_main = st.text_input("검색 (메인: 단품코드/명칭)", key="search_main")

# -----------------------
# 탭별 필터 (사이드바 expander)
# -----------------------
with st.sidebar.expander("시계열 추이 필터", expanded=False):
    ts_mode = st.radio("표시 기준 (시계열)", ("브랜드별", "시리즈별"), key="ts_mode")
    ts_choices = (sorted(mg_all['brand'].unique()) if ts_mode == "브랜드별"
                  else sorted(mg_all['series'].unique()))
    ts_target = st.multiselect("표시할 항목 선택 (시계열)", ts_choices, default=None,
                               key="ts_target")

with st.sidebar.expander("시리즈 상세 필터", expanded=False):
    sel_brand_series = st.selectbox("브랜드 선택 (시리즈 상세)",
                                    ["전체"] + all_brands, index=0,
                                    key="brand_series")
    sel_supply_series = st.selectbox("공급단 선택 (시리즈 상세)",
                                     ["전체"] + supply_values, index=0,
                                     key="supply_series")
    top_n_series = st.slider("표시할 시리즈 수 (Top N, 시리즈 상세)", 5, 50, 20,
                             key="topn_series")

with st.sidebar.expander("전체 데이터 필터", expanded=False):
    sel_supply_all = st.selectbox("공급단 선택 (전체 데이터)",
                                  ["전체"] + supply_values, index=0,
                                  key="supply_all")

with st.sidebar.expander("수주대비 실적 분석 필터", expanded=False):
    perf_threshold_low  = st.number_input("과소예측 기준(달성률 미만)",  value=90,
                                          min_value=1,   max_value=100,  key="perf_low")
    perf_threshold_high = st.number_input("과대예측 기준(달성률 초과)", value=110,
                                          min_value=100, max_value=1000, key="perf_high")
...
with tab_main:
    st.header("메인 대시보드")
    st.caption("사이드바 → 📂 메인 대시보드 필터 에서 상세 조건을 설정하세요.")

    if not sel_sr_main:
        st.warning("사이드바 '메인 대시보드 필터'에서 시리즈를 선택하세요.")
    else:
        # ★ 핵심 수정 2: 공급단 필터를 forecast 기준이 아닌 mg_all(병합 후) 기준으로 적용
        #    → supply 컬럼은 f_df(예측) 쪽에만 있으므로, mg_all 병합 시 이미 포함됨
        #    → '전체' 선택 시 supply 필터 자체를 적용하지 않아 모든 공급단 데이터 포함
        df_main = mg_all[
            (mg_all["ym"] == sel_ym) &
            (mg_all["brand"].isin(sel_br)) &
            (mg_all["series"].isin(sel_sr_main))
        ].copy()

        # 공급단 필터: '전체'가 아닐 때만 적용
        ...
        sort_col, sort_asc = sort_map[sort_metric_main]
        df_main = df_main.sort_values(by=sort_col, ascending=sort_asc)

        # 검색 필터
        if search_term_main:
            mask = (
                df_main['combo'].str.contains(search_term_main, case=False, na=False) |
                df_main['name'].str.contains(search_term_main, case=False, na=False)
            )
            df_main = df_main[mask]

        # 공급단별 현황 디버그 정보 (개발자용 → 필요 없으면 제거 가능)
        supply_counts = df_main['supply'].fillna('(미지정)').value_counts()
        with st.expander("📊 공급단별 포함 현황 확인", expanded=False):
            st.dataframe(supply_counts.reset_index().rename(
                columns={'index': '공급단', 'supply': '건수'}
            ))

        # 요약 카드
        t_f = int(df_main['forecast'].sum())
        t_a = int(df_main['actual'].sum())
        t_d = int(t_a - t_f)
        ...
        with m1:
            st.markdown(
                ...
                f'<div class="metric-label">예측수요 합계</div>'
                f'<div class="metric-value">{t_f:,}</div>'
                f'<div class="metric-sub">(역산)</div></div>',
                unsafe_allow_html=True
            )
        with m2:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">실수주량 합계</div>'
                f'<div class="metric-value">{t_a:,}</div>'
                f'<div class="metric-sub">{sel_ym.split("-")[1]}월</div></div>',
                unsafe_allow_html=True
            )
        with m3:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">차이량 합계</div>'
                f'<div class="metric-value" style="color:#fb7185">{t_d:,}</div>'
                f'<div class="metric-sub">예측대비 차이</div></div>',
                unsafe_allow_html=True
            )
        with m4:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">전체 달성률</div>'
                f'<div class="metric-value">{t_r:.1f}%</div>'
                f'<div class="metric-sub">실수주 / 예측</div></div>',
                unsafe_allow_html=True
            )

        st.write("")
        c1, c2 = st.columns(2)
        chart_data = df_main.head(top_n_main)

        with c1:
            st.subheader(f"상위 Top {top_n_main} 수량 분석")
            fig1 = go.Figure()
            fig1.add_trace(go.Bar(x=chart_data['series'], y=chart_data['forecast'],
                                  name='예측수요', marker_color='#3b82f6'))
            fig1.add_trace(go.Bar(x=chart_data['series'], y=chart_data['actual'],
                                  name='실수주량', marker_color='#fb7185'))
            fig1.add_trace(go.Bar(x=chart_data['series'], y=chart_data['차이'],
                                  name='차이량', marker_color='#f59e0b'))
        ...
        with c2:
            ...
            fig2 = go.Figure()
            fig2.add_trace(go.Bar(x=chart_data['series'], y=chart_data['달성률(%)'],
                                  name='달성률', marker_color='#0ea5e9'))
        ...
        display_df = df_main[cols_show].copy().fillna("")
        st.subheader("상세 데이터")
        st.dataframe(
            style_number_df(display_df,
                            int_cols=['forecast','actual','차이'],
                            float_cols=['달성률(%)']),
...
with tab_ts:
    st.header("시계열 추이")
    st.caption("사이드바 → 📂 시계열 추이 필터 에서 조건을 설정하세요.")
    ...
    agg = mg_time.groupby(['ym_dt', group_col]).agg(
    ...
    if ts_target:
        ...
        top_items = (
            mg_time.groupby(group_col)['forecast']
            .sum().abs().sort_values(ascending=False).head(4).index.tolist()
        )
    ...
    if agg.empty:
        ...
        for it in agg[group_col].unique():
            ...
            fig.add_trace(go.Scatter(x=df_it['ym_dt'], y=df_it['forecast'],
                                     mode='lines+markers', name=f"{it} 예측",
                                     line=dict(dash='dash')))
            fig.add_trace(go.Scatter(x=df_it['ym_dt'], y=df_it['actual'],
                                     mode='lines+markers', name=f"{it} 실적"))
        fig.update_layout(title="월별 예측 vs 실적 추이",
                          xaxis_title="기준월", yaxis_title="수량",
...
with tab_series:
    st.header("시리즈 상세")
    st.caption("사이드바 → 📂 시리즈 상세 필터 에서 조건을 설정하세요.")
    ...
    series_agg = (
        df_series.groupby('series')
        .agg({'forecast': 'sum'}).reset_index()
    ...
    if series_agg.empty:
        ...
        fig_s = go.Figure(go.Bar(x=plot_df['forecast'], y=plot_df['series'],
                                 orientation='h', marker_color='#3b82f6'))
        fig_s.update_layout(title=f"시리즈별 예측량 (Top {top_n_series})",
                            xaxis_title="예측수량", yaxis_title="시리즈",
                            template='plotly_white', height=520)
        ...
        merged_series['달성률(%)'] = np.where(
            merged_series['forecast'] > 0,
            (merged_series['actual'] / merged_series['forecast'] * 100).round(1), 0
        )
        st.subheader("시리즈별 예측/실적 (전체 기간 합계)")
        st.dataframe(
            style_number_df(
                merged_series.rename(columns={'forecast': '예측수요', 'actual': '실수주'}),
                int_cols=['예측수요', '실수주'],
                float_cols=['달성률(%)']
            ),
            use_container_width=True
...
with tab_all:
    st.header("전체 데이터")
    st.caption("사이드바 → 📂 전체 데이터 필터 에서 조건을 설정하세요.")
    ...
    supply_agg = (
        df_all.dropna(subset=['supply'])
        .groupby('supply').agg({'forecast': 'sum'}).reset_index()
    )
    ...
    if supply_agg.empty or total_forecast == 0:
        ...
        fig_pie = go.Figure(data=[go.Pie(
            labels=supply_agg['supply'],
            values=supply_agg['forecast'],
            hole=0.45,
            marker=dict(colors=['#60a5fa', '#fb7185', '#34d399', '#f59e0b'])
        )])
        fig_pie.update_layout(
            title=f"공급단별 예측 비중 (총합: {total_forecast:,})",
            height=420, template='plotly_white'
        )
        st.plotly_chart(fig_pie, use_container_width=True)

        pivot = df_all.dropna(subset=['supply']).pivot_table(
            index='brand', columns='supply',
        ...
        st.dataframe(
            style_number_df(
                pivot.astype(int).reset_index(),
                int_cols=pivot.columns.tolist()
            ),
            use_container_width=True
        )
...
def generate_narrative(mg_perf, low_thr=90, high_thr=110):
    ...
    series_perf['달성률'] = np.where(
        series_perf['forecast'] > 0,
        (series_perf['actual'] / series_perf['forecast'] * 100).round(1), 0
    )
    ...
    <div class="analysis-box">
      ...
      - 전체적으로 달성률이 {total_rate:.1f}%로 나타나 예측 대비 실수주가
        {'부족' if total_rate < 100 else '초과'}한 경향이 있습니다.<br>
    ...
    if worst.empty:
        ...
        for _, row in worst.iterrows():
            html += (f"- {row['series']}: 예측 {int(row['forecast']):,} → "
                     f"실제 {int(row['actual']):,} "
                     f"(오차 {int(row['오차량']):,}, 달성률 {row['달성률']:.1f}%)<br>")

    html += f"<br><strong>과소/과대 예측 요약</strong><br>"
    if under.empty:
        html += f"- 과소예측(달성률 &lt; {low_thr}%) 항목: 없음<br>"
    else:
        html += f"- 과소예측(달성률 &lt; {low_thr}%) 상위: "
        html += ", ".join([f"{r['series']}({r['달성률']}%)" for _, r in under.iterrows()]) + "<br>"

    if over.empty:
        html += f"- 과대예측(달성률 &gt; {high_thr}%) 항목: 없음<br>"
    else:
        html += f"- 과대예측(달성률 &gt; {high_thr}%) 상위: "
    ...
    html += """
      ...
      2. 과소예측 품목은 수요 감소 원인(반품·납기지연·채널 축소 등)을 점검하고,
         과대예측 품목은 판촉·대량발주 여부를 확인하세요.<br>
      3. 다음 예측 주기에는 상위 변동 시리즈에 대해 최근 3개월 추세를 반영하거나
         가중치를 조정하세요.<br>
      4. 반복적으로 오차가 발생하는 시리즈는 별도 모니터링 대상으로 지정해 알림을 설정하세요.<br>
    </div>
    """
    return html
...
with tab_perf:
    ...
    st.caption("사이드바 → 📂 수주대비 실적 분석 필터 에서 조건을 설정하세요.")

    mg_perf = mg_all[
        (mg_all['ym'] == sel_ym) & (mg_all['brand'].isin(sel_br))
    ].copy()
    if sel_supply_main != "전체":
    ...
    if mg_perf.empty:
        ...
        st.dataframe(
            style_number_df(
                series_perf.rename(columns={'forecast':'예측수요','actual':'실수주'}),
                int_cols=['예측수요','실수주','오차량'],
                float_cols=['달성률(%)']
            ),
            use_container_width=True
        )
        ...
        narrative_html = generate_narrative(mg_perf,
                                            low_thr=perf_threshold_low,
                                            high_thr=perf_threshold_high)
...
preview = mg_all[
    (mg_all['brand'].isin(sel_br)) & (mg_all['ym'] == sel_ym)
].copy()
if sel_supply_main != "전체":
...
if preview.empty:
    ...
    cols_show = ['ym','brand','series','combo','name','supply',
                 'forecast','actual','차이','달성률(%)']
    preview = preview[cols_show].copy().fillna("")
    st.dataframe(
        style_number_df(preview,
                        int_cols=['forecast','actual','차이'],
                        float_cols=['달성률(%)']),
        use_container_width=True
    )
